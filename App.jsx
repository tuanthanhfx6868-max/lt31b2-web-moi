...
function useRole(user, isAdminLogin) {
  ...
  const isCommandRole = Boolean(rosterMatch && (rosterMatch.role === "Trung đội trưởng" || rosterMatch.role === "Trung đội phó"));
...
function RosterTab({ perm }) {
  ...
  const [form, setForm] = useState({ msv: "", name: "", role: "Cán bộ", tieuDoi: "1", phone: "" });
  const [showForm, setShowForm] = useState(false);

  const add = async () => {
    if (!form.name.trim()) return;
    await setItems([...items, { id: Date.now(), ...form }]);
    setForm({ msv: "", name: "", role: "Cán bộ", tieuDoi: "1", phone: "" });
    setShowForm(false);
  };
  const remove = async (id) => setItems(items.filter((i) => i.id !== id));
        ...
        <div className="stamp-border p-4 mb-5 grid grid-cols-1 md:grid-cols-2 gap-3" style={{ background: "#fff" }}>
          ...
          <Field label="Chức vụ">
            <select className={inputCls} style={inputStyle} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
              <option>Trung đội trưởng</option><option>Trung đội phó</option>
              <option>Tiểu đội trưởng</option><option>Tiểu đội phó</option>
              <option>Bí thư chi bộ</option><option>Phó bí thư chi bộ</option>
              <option>Chi uỷ viên chi bộ</option><option>Thư ký chi bộ</option>
              <option>Bí thư chi đoàn</option><option>Phó bí thư chi đoàn</option>
              <option>Uỷ viên chi đoàn</option><option>Cán bộ</option>
            </select>
          </Field>
            ...
            <tbody>
              {items.map((m, i) => (
                <tr key={m.id} style={{ background: i % 2 ? T.paper : "#fff" }}>
                  ...
                  <td className="px-3 py-2 text-right">{perm.canManage && <button onClick={() => remove(m.id)}><Trash2 size={14} style={{ color: T.red }} /></button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
